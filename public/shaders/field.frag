...
void main() {
  ...
  float density = 0.45 + u_state * 0.4;

  vec3 col = BG;

  // a few drifting orbs, accent → purple by depth
  for (int i = 0; i < 4; i++) {
    float fi = float(i);
    float a = t + fi * 1.7;
    vec2 c = vec2(sin(a * 1.3 + fi) * 0.6, cos(a + fi * 0.8) * 0.4 - 0.1);
  ...
  col += ACCENT * line * 0.012 * (1.0 - scroll * 0.6);
